      ...
      <div className="h-full flex flex-col overflow-auto gap-5 p-5">
        {
          isChatLoading ? <div>Loading...</div> : chatMessages.map((message) => {
            return (
              <div key={message._id} className={`flex flex-col ${message.senderId === selectedUser._id ? "items-start" : "items-end"}`}>
                <div className={`flex items-center gap-2 ${message.senderId === selectedUser._id ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className="items-start">
                    <img src={message.senderId === selectedUser._id ? selectedUser?.profileImage : message.sender?.profileImage || "/avatarDemo.png"} className="size-12 rounded-full object-cover" alt="" />
                  </div>
                </div>
              </div>
            )
